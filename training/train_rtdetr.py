# ============================================================================
# train_rtdetr.py — paste cell-by-cell into Google Colab (Runtime -> GPU, T4)
#
# Trains RT-DETR-l across 3 seeds on the unified 22-class fossil-pollen
# dataset (depth-based, leakage-free split). Same protocol and
# hyperparameters as the YOLO runs (see train_yolo.py / experiment_config.yaml)
# so the transformer baseline is directly comparable.
#
...
#
# In practice the 3 seeds were trained across separate Colab/Kaggle sessions;
# this script trains/resumes whichever seeds in SEEDS are not yet marked
# COMPLETED on Drive.
#
...
for seed in SEEDS:
    ...
    if os.path.exists(csv):
        ...
        print(f"  seed {seed}: mAP50 = {rows[seed]['mAP50']:.3f}")
    else:
        print(f"  seed {seed}: results.csv not on Drive yet")

print(f"\n{'='*60}\nRT-DETR-l -- summary ({len(rows)}/{len(SEEDS)} seeds ready)\n{'='*60}")
